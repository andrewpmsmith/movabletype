...
public class WordList extends SQLiteOpenHelper{
	...
	/* Prepare a word for storage in the DB.
	 * Reject words less than two letters in length.
	 * Reject words with non-alphabetical characters (e.g. apostrophes/hyphens).
	...
	public void addWords(SQLiteDatabase db) {
		...
		InputStream is = mContext.getResources().openRawResource(R.raw.wordlist);
	...
	public boolean wordInDictionary(String word) {
		...
		Cursor c = db.rawQuery("SELECT 1 FROM " + TABLE_WORDS + " WHERE " + KEY_WORD + "= '" + word + "'", null);
