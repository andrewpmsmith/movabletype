...
public class Widget {
	...
	WidgetClickObserver mClickObserver = null;
	WidgetDragObserver mDragObserver = null;
	...
	public synchronized void setShadow(int radius, int color) {
		// mPaint must not be accessed while drawing as it will casue a segmentation fault
	...
	public void setClickObserver(WidgetClickObserver clickObserver) {
		mClickObserver = clickObserver;
	}
	
	public void setDragObserver(WidgetDragObserver dragObserver) {
		mDragObserver = dragObserver;
	}
	
	public boolean onTouchEvent(MotionEvent m) {
		
		if (mClickObserver!=null || mDragObserver!=null) {
			...
			if (m.getAction() == MotionEvent.ACTION_DOWN) {
			...
			} else if (m.getAction() == MotionEvent.ACTION_UP || m.getAction() == MotionEvent.ACTION_CANCEL) {
				
				if (mDragInProgress) {
					
					if (mDragObserver!=null) mDragObserver.onDragEnd(this);
					mDragInProgress = false;
					
				} else {
					
					if (mClickObserver!=null) mClickObserver.onClick(this);
				...
				if (mDragInProgress) {
					...
					if (mDragObserver!=null) mDragObserver.onDrag(this, newX, newY);
					...
					if (Math.hypot(dX, dY)>DRAG_THRESHOLD) {
						...
						if (mDragObserver!=null) mDragObserver.onDragStart(this);
					}
				}
			}
