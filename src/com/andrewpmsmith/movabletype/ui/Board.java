...
public class Board extends RenderSurface
	...
	private enum DragAnimation {NONE, EXPANDING, CONTRACTING};
	...
	public void init() {
		...
		for (int i=0; i<GameModel.GRID_ITEMS; ++i) {
			...
			mTiles[i].setClickObserver(this);
			mTiles[i].setDragObserver(this);
			addWidget(mTiles[i]);
		}
		...
		mClearButton = new TextWidget(COLOR_INVISIBLE, clearButtonText, Color.BLACK);
		mClearButton.setClickObserver(new WidgetClickObserver() {
		...
		mPlayButton = new TextWidget(COLOR_INVISIBLE, submitButtonText, Color.BLACK);
		mPlayButton.setClickObserver(new WidgetClickObserver() {
		...
		String player1Score = String.valueOf(mGameModel.getPoints(GameModel.PLAYER1));
		String player2Score = String.valueOf(mGameModel.getPoints(GameModel.PLAYER2));
		
		mPlayer1Score = new TextWidget(COLOR_INVISIBLE, player1Score, mPlayer1SurroundedColor);
		mPlayer2Score = new TextWidget(COLOR_INVISIBLE, player2Score, mPlayer2SurroundedColor);
	...
	public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		final double measuredHeightToWidthRatio = 
				(double)measuredHeight/measuredWidth;
	...
	private void layoutBoard() {
		...
		mPlayer1Score.applyLayout(getWidth()/2 - mTileWidthInWord, 0,
				mTileWidthInWord, mTileWidthInWord);
		mPlayer2Score.applyLayout(getWidth()/2, 0,
				mTileWidthInWord, mTileWidthInWord);
	...
	public void onClick(Widget w) {
		...
		if (w.getY() == mWordTop) {
			...
			animateToPosition(t, t.mPositionInGrid_x, t.mPositionInGrid_y, Tile.widthInGrid);
	...
	public void onDragStart(Widget w) {
		...
		float angle = (mRand.nextFloat() * (Math.abs(MIN_DRAG_ROTATION) + Math.abs(MAX_DRAG_ROTATION) )) + MIN_DRAG_ROTATION ;
	...
	public void onDragEnd(Widget w) {
		...
		if (w.getY() <= mAddToWordThreshold && mPlaceHolderIndex!=PLACEHOLDER_UNUSED) {
	...
	public void onDrag(Widget w, int x, int y) {
		...
		if (y<mGridTop) {
			...
			int word_begin = (int)(getWidth()/2 - (Tile.widthInWord/2.0) * mWord.size());
			...
			if (x<word_begin) {
				...
				index= (int)(x- (word_begin - Tile.widthInWord/2.0))/Tile.widthInWord;
			}
	...
	private void expandContractTile(Tile t, int newWidth) {
		if (mExpandContractAnimation!=null) t.cancelAnimation(mExpandContractAnimation);
		mExpandContractAnimation = new ExpandContractAnimation(t, ANIMATION_DURATION, newWidth);
	...
	private void presentWord() {
		...
		int offset = (int)(getWidth()/2 - (Tile.widthInWord/2.0) * mWord.size());
		...
		mPlayer1Score.setText( String.valueOf(mGameModel.getPoints(GameModel.PLAYER1)) );
		mPlayer2Score.setText( String.valueOf(mGameModel.getPoints(GameModel.PLAYER2)) );
		...
		if (turnResult == GameModel.TurnResult.SUCCESS) {
			...
			if (state != GameModel.GameState.GAME_OVER) {
				...
				message = String.format(message,
						(state==GameModel.GameState.PLAYER1_TURN)?"1":"2");
				
				int playerColor = state==GameModel.GameState.PLAYER1_TURN
						? mPlayer1Color
						: mPlayer2Color;
				...
				if (result == GameModel.GameResult.DRAW) {
					...
					String message = res.getString(R.string.game_over_draw);
					...
					String message = res.getString(R.string.game_over);
					message = String.format(message,
							(result == GameModel.GameResult.PLAYER1_WIN)?"1":"2");
			...
			case WORD_LESS_THAN_TWO_LETTERS:
				message = getResources().getString(R.string.word_too_short_error);
				break;
